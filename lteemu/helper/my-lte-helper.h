...
#ifndef MY_LTE_HELPER_H
#define MY_LTE_HELPER_H
 ...
 *
 */
class MyLteHelper : public Object
{
public:
  MyLteHelper (void);
  virtual ~MyLteHelper (void);
  ...
  /** 
   * Set the MyEpcHelper to be used to setup the EPC network in
   * conjunction with the setup of the LTE radio access network 
   *
   * \note if no MyEpcHelper is ever set, then MyLteHelper will default
   ...
   * the RLC.
   * 
   * \param h a pointer to the MyEpcHelper to be used
   */
  void SetEpcHelper (Ptr<MyEpcHelper> h);
   ...
   * Will call MyLteHelper::Attach () passing to it the single eNB
