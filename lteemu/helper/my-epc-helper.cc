...
#include <ns3/my-epc-sgw-pgw-application.h>
...
NS_LOG_COMPONENT_DEFINE ("MyEpcHelper");

NS_OBJECT_ENSURE_REGISTERED (MyEpcHelper);


MyEpcHelper::MyEpcHelper () 
  : m_gtpuUdpPort (2152)  // fixed by the standard
{
  ...
  // create MyEpcSgwPgwApplication
  m_sgwPgwApp = CreateObject<MyEpcSgwPgwApplication> (m_tunDevice, sgwPgwS1uSocket);
  m_sgwPgw->AddApplication (m_sgwPgwApp);
  
  // connect SgwPgwApplication and virtual net device for tunneling
  m_tunDevice->SetSendCallback (MakeCallback (&MyEpcSgwPgwApplication::RecvFromTunDevice, m_sgwPgwApp));
...
TypeId
MyEpcHelper::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MyEpcHelper")
    .SetParent<Object> ()
    .AddConstructor<MyEpcHelper> ()
    .AddAttribute ("S1uLinkDataRate", 
                   "The data rate to be used for the next S1-U link to be created",
                   DataRateValue (DataRate ("10Gb/s")),
                   MakeDataRateAccessor (&MyEpcHelper::m_s1uLinkDataRate),
                   MakeDataRateChecker ())
    .AddAttribute ("S1uLinkDelay", 
                   "The delay to be used for the next S1-U link to be created",
                   TimeValue (Seconds (0)),
                   MakeTimeAccessor (&MyEpcHelper::m_s1uLinkDelay),
                   MakeTimeChecker ())
    .AddAttribute ("S1uLinkMtu", 
                   ...
                   UintegerValue (2000),
                   MakeUintegerAccessor (&MyEpcHelper::m_s1uLinkMtu),
                   MakeUintegerChecker<uint16_t> ())
    .AddAttribute ("X2LinkDataRate",
                   "The data rate to be used for the next X2 link to be created",
                   DataRateValue (DataRate ("10Gb/s")),
                   MakeDataRateAccessor (&MyEpcHelper::m_x2LinkDataRate),
                   MakeDataRateChecker ())
    .AddAttribute ("X2LinkDelay",
                   "The delay to be used for the next X2 link to be created",
                   TimeValue (Seconds (0)),
                   MakeTimeAccessor (&MyEpcHelper::m_x2LinkDelay),
                   MakeTimeChecker ())
    .AddAttribute ("X2LinkMtu",
                   ...
                   UintegerValue (3000),
                   MakeUintegerAccessor (&MyEpcHelper::m_x2LinkMtu),
                   MakeUintegerChecker<uint16_t> ())
  ;
  return tid;
}
...
void
MyEpcHelper::AddEnb (Ptr<Node> enb, Ptr<NetDevice> lteEnbNetDevice, uint16_t cellId)
{
...
void
MyEpcHelper::AddX2Interface (Ptr<Node> enb1, Ptr<Node> enb2)
{
...
void 
MyEpcHelper::AddUe (Ptr<NetDevice> ueDevice, uint64_t imsi)
{
...
void
MyEpcHelper::ActivateEpsBearer (Ptr<NetDevice> ueDevice, uint64_t imsi, Ptr<EpcTft> tft, EpsBearer bearer)
{
...
MyEpcHelper::AssignUeIpv4Address (NetDeviceContainer ueDevices)
{
...
Ipv4Address
MyEpcHelper::GetUeDefaultGatewayAddress ()
{
